# Landing-page copy audit

Audited 2 September 2026. Counts treat hyphenated terms and `6–12` as one
word. No line exceeds 22 words, and no line uses a banned marketing word.

| Landing text | Words |
|---|---:|
| For volunteer math circle facilitators | 5 |
| Plan and record small math-circle sessions | 6 |
| Sequence open problems, record partial attempts, keep private notes, and print a recap for 6–12 learners. | 16 |
| Try it with sample data | 5 |
| See a filled board. | 5 |
| Changes stay in this demo. | 5 |
| Sign in with Microsoft | 4 |
| Open the board owned by your account. | 7 |
| Set up a board with the adult owner code. | 9 |
| Private boards require the owner’s Microsoft sign-in. | 7 |
| Sample mode reloads offline after its first visit. | 8 |
| All current board tools are free. | 6 |
| Sample problem cards on a facilitator’s table. | 7 |
| See one session at a glance | 6 |
| The board keeps the problem order, learner status, and next discussion prompt together. | 12 |
| Open this sample board | 4 |
| Run the session in three steps | 6 |
| Add prompts in the order you plan to discuss them. | 10 |
| Save partial ideas, strategy tags, and a private facilitator note. | 10 |
| Make a session record that leaves private notes out. | 9 |
| Data kept on the board | 5 |
| Use aliases instead of learner emails. | 6 |
| The board is not a gradebook, chat service, or public learner profile. | 12 |
| Read the privacy details | 4 |
| This release is for one private circle | 8 |
| It has no paid plan, checkout, organization controls, or extra storage tier. | 12 |
| Use four free strategy prompts | 5 |
| Plan and record small math-circle sessions. | 6 |
| Privacy | 1 |
| Terms | 1 |
| Built by Param Factory | 4 |
| v0.1.0 | 1 |

## README corrections

The review’s flagged test sentence is now two sentences of 16 and 11 words.
The demo-isolation command now says “named demo-isolation check” instead of
“manifest claim.” No README sentence exceeds the 22-word limit.

## Catalog description

“Record partial learner attempts, private notes, and printable recaps for a small math
circle.” has 14 words and 93 characters. It starts with a verb. Its observable
capabilities map to the `attempt-record` and `recap-privacy` claim checks.

## Terminology

| Concept | Word used |
|---|---|
| A scheduled gathering | session |
| A participant record | learner |
| A learner’s partial work | attempt |
| Facilitator-only writing | private note |
| Browser-only sample | demo / sample mode |
| Printable session record | recap |
| Reusable thinking starter | strategy prompt |
| Product boundary | release scope |
